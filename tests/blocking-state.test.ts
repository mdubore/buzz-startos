...
test('valid stable state without setup selects only initial setup', () => {
  assert.deepEqual(
    selectBlockingTask({ kind: 'needs-setup', state: VALID_SECRETS }, [
      'https://buzz.example',
    ]),
    {
...
test('ready state with an unavailable canonical URL selects URL recovery', () => {
...
test('ready state with its normalized canonical URL clears every task', () => {
  assert.deepEqual(selectBlockingTask(READY, ['https://BUZZ.EXAMPLE.:443/']), {
    actionId: null,
    replayId: null,
    clearReplayIds: [
      STATE_RECOVERY_TASK_REPLAY_ID,
