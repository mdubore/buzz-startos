...
buzz_ref="ghcr.io/block/buzz:sha-$short_sha"
...
```bash
docker buildx imagetools inspect '<exact-tag>'
docker buildx imagetools inspect '<exact-tag>' --raw |
  jq -r '.manifests[]
