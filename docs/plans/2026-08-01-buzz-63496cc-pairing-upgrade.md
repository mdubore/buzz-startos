...
- Preserve unchanged: the source production-readiness worktree referenced by
  `SOURCE_WORKTREE` below
...
```bash
PACKAGE_REPO='/path/to/buzz-startos'
SOURCE_WORKTREE='/path/to/production-readiness-worktree'
INTEGRATION_WORKTREE='/path/to/63496cc-pairing-upgrade-worktree'

git -C "$PACKAGE_REPO" worktree add \
...
If the configured StartOS hostname has unreliable DNS or mDNS resolution, set
`STARTOS_BUILD_HOST` to a reachable StartOS HTTPS origin covered by its TLS
certificate. Replace the reserved example before use, then confirm the endpoint
before packing:

```bash
STARTOS_BUILD_HOST='https://startos-build-host.example' # Replace before use.
start-cli -H "$STARTOS_BUILD_HOST" echo build-host-check
```
...
Do not rely on `make x86` while the configured hostname is unresolved: its
hidden `list-ingredients` call can produce an empty prerequisite list and reuse
stale JavaScript.

```bash
make javascript/index.js
node -e "const p=require('./javascript/index.js'); console.log(p.manifest.version)"
start-cli -H "$STARTOS_BUILD_HOST" s9pk pack \
  --arch=x86_64 \
...
```bash
start-cli -H "$STARTOS_BUILD_HOST" s9pk inspect \
...
```bash
PACKAGE_ROOT="$(pwd -P)"
ARTIFACT="$PACKAGE_ROOT/buzz_x86_64.candidate.s9pk"
cd /tmp
"$PACKAGE_ROOT/scripts/verify-s9pk-signer.sh" "$ARTIFACT"
sha256sum "$ARTIFACT"
```
